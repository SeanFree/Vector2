let Vector2=((Math)=>{class Vector2{constructor(x,y){this.x=x||0;this.y=y||0;return this}
...
getMagnitude(){return Math.sqrt(Math.pow(this.x,2)+Math.pow(this.y,2))}
normalize(){this.divideScalar(this.getMagnitude())}
randomize(bounds){bounds=bounds||new Vector2(1,1);this.x=Math.random()*bounds.x;this.y=Math.random()*bounds.y;return this}
addRandom(limit){this.x+=limit-Math.random()*(limit*2);this.y+=limit-Math.random()*(limit*2)}
addRandomX(limit){this.x+=limit-Math.random()*(limit*2)}
addRandomY(limit){this.y+=limit-Math.random()*(limit*2)}
